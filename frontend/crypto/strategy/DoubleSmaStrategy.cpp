...
std::optional<Signal> DoubleSmaStrategy::push_price(std::pair<std::chrono::milliseconds, double> ts_and_price)
{
    ...
    m_strategy_internal_data_publisher.push(ts_and_price.first, {"prices", "fast_avg_history", current_average_fast});
    ...
    m_strategy_internal_data_publisher.push(ts_and_price.first, {"prices", "slow_avg_history", current_average_slow});
...
EventTimeseriesPublisher<std::tuple<std::string, std::string, double>> & DoubleSmaStrategy::strategy_internal_data_publisher()
{
